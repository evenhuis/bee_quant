...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	var_dict[dvar_l0s] = pm.HalfNormal   ( dvar_l0s,       sd=lv0_sd  )
	var_dict[dvar_l0 ] = pm.Deterministic( dvar_l0, var_dict[dvar_l0s]*pm.Normal(dvar_l0c, mu=0,sd=1,shape=nind) )
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	dvar_l0 = 'd'+var+'_l0'; dvar_l0c  = dvar_l0 +'c'
	dvar_l0s = dvar_l0+'s' ; dvar_l0sc = dvar_l0s+'s'; dvar_l0st=dvar_l0s+'t'
	n2 = len(index)
	if( dist=='halfnorm'):
		var_dict[dvar_l0st] = pm.HalfNormal(dvar_l0st, sd=lv0_sd )
		var_dict[dvar_l0s ] = pm.Deterministic( dvar_l0s,   var_dict[dvar_l0st]	*pm.HalfNormal( dvar_l0sc,sd=1,shape=n1)  )	
	if( dist=='gamma'):
